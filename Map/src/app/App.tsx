...
export default function App() {
  const [storeType, setStoreType] = useState<StoreType>('beverage');
  const [weights, setWeights] = useState<WeightConfig>(getDefaultWeights('beverage'));
  const [selectedLocation, setSelectedLocation] = useState<LocationData | null>(null);
  const [comparisonLocations, setComparisonLocations] = useState<LocationData[]>([]);
  ...
  const handleAddToComparison = () => {
    if (selectedLocation && comparisonLocations.length < 3) {
      if (!comparisonLocations.find(loc => loc.id === selectedLocation.id)) {
